        ...
        public static string GetStreamSubjects(string? url) //Maybe better to collect the subjects from consumers?
        {
            ...
            foreach (string[] element in listOfSubjectArray)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    {
                        string? parentName = i - 1 < 0 ? null : element[i - 1];
                        string? childName = i + 1 > element.Length - 1 ? null : element[i + 1];
                        SubjectManager.AddSubject(element[i], parentName, childName);
                    }
                }
            }
            
            SubjectManager.GetHierarchy();
